...
public class MyMenu extends JMenuBar {
    ...
    public MyMenu(Controlador c) {
        ...
        try {
            Image img = ImageIO.read(classLoader.getResourceAsStream(UNDO_ICON));
        ...
        try {
            Image img = ImageIO.read(classLoader.getResourceAsStream(REDO_ICON));
            rehacer.setIcon(new ImageIcon(img));
        } catch (Exception ex) {
            System.out.println(ex);
        }
        rehacer.setForeground(theme.fontColor());
        if (c.getContFicherosDeshacer() == c.getLimiteFicherosDeshacer() || c.getAuxDeshacer() == true)
            rehacer.setBackground(Color.GRAY);
