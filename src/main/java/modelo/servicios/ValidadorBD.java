...
public class ValidadorBD extends GeneradorEsquema {
    ...
    private void error(Transfer t, String texto) {
        if (t instanceof TransferEntidad)
            texto = Lenguaje.text(Lenguaje.THE_ENTITY) + " " + t.getNombre() + " " + texto;
        else if (t instanceof TransferRelacion)
            texto = Lenguaje.text(Lenguaje.THE_RELATION) + " " + t.getNombre() + " " + texto;
            ...
            texto = Lenguaje.text(Lenguaje.THE_DOMAIN) + " " + t.getNombre() + " " + texto;
        errores += "<li>" + texto + "</li>";
    }
    ...
    private boolean validaComponentesRelacionDebil(TransferRelacion tr) {
        ...
        switch (tam) {
            case 0:
                error(tr, Lenguaje.text(Lenguaje.NO_ENT_RELATION));
                valida = false;
                break;
            case 1:
                error(tr, Lenguaje.text(Lenguaje.ONE_ENT_WEAK_REL));
                valida = false;
                break;
            default:
    ...
    private void validaFidelidadEntidadEnIsA(TransferEntidad te) {
        ...
        if (papi > 1)
            warning(te, Lenguaje.text(Lenguaje.ENT_PARENT_IN) + " " + papi + Lenguaje.text(Lenguaje.ISA_RELATIONS));
    }
    ...
    private boolean validaComponentesRelacionIsA(TransferRelacion tr) {
        ...
        if (dameNumEntidadesDebiles(tr) > 0) {
            ...
            switch (tam) {
                case 0:
                    error(tr, Lenguaje.text(Lenguaje.NO_PARENT_REL));
                    valida = false;
                    break;
                case 1:
                    error(tr, Lenguaje.text(Lenguaje.NO_CHILD_RELATION));
                    valida = false;
                    break;
                default:
                    break;
        ...
        if (dameNumEntidadesDebiles(tr) > 0 && !this.misDebilesEstanEnDebiles(tr)) {
            ...
            switch (tam) {
                case 0:
                    ...
                    break;
                case 1:
                    error(tr, Lenguaje.text(Lenguaje.ONE_ENT_REL));
                    valida = false;
                    break;
                default:
                    break;
    ...
    private boolean validaCompuesto(TransferAtributo ta) {
        ...
        switch (numSubs) {
            case 0:
                ...
                break;
            case 1:
                warning(ta, Lenguaje.text(Lenguaje.ONE_SUBATTRIB));
                break;
        }
